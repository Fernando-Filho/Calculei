function inss( baseCalculo )
{
    ...
    {
        valorINSS = ((baseCalculo * inss.faixa1[alíquota]) / 100)
    }
    else if(baseCalculo <= inss.faixa2[valorFinal])
    {
        valorINSS =  ((((baseCalculo - inss.faixa1[valorFinal]) * inss.faixa2[alíquota]) / 100) + inss.faixa1[descontoMáximoAtéFaixa])
    }
    else if(baseCalculo <= inss.faixa3[valorFinal])
    {
        valorINSS =  ((((baseCalculo - inss.faixa2[valorFinal]) * inss.faixa3[alíquota]) / 100) + inss.faixa2[descontoMáximoAtéFaixa])
    }
    else if(baseCalculo <= inss.faixa4[valorFinal])
    {
        valorINSS =  ((((baseCalculo - inss.faixa3[valorFinal]) * inss.faixa4[alíquota]) / 100) + inss.faixa3[descontoMáximoAtéFaixa])
    }
    else{ valorINSS = inss.faixa4[descontoMáximoAtéFaixa] }
